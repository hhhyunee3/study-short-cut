...
import { esc, rhash, pick, fill, imgPH, bodyImg, subjInfo, GRADE_INFO, ABS, faqHtml, ctaHtml, updHtml, pillsHtml, bcHtml, serviceLD } from './core.js';
import { schoolSections, pools } from './gen.js';
...
${bodyImg('schools-hub', '학교별 수업', '5/2')}
...
export function schoolPage(slug) {
  ...
  const ctx = schoolCtx(s); const L = s.level;
  const canonical = ABS(`/school/${s.slug}/`);
  const seed = 'school-' + slug;
  const G = schoolSections(L, seed, ctx, '');
  if (!G) return null;
  const subjGrid = SITE.subjects.map((k, i) => { const si = subjInfo(k); return `<a href="/school/${s.slug}/${k}/"${i === 0 ? ' class="pop"' : ''}>${si.emoji} ${esc(si.kor)}<small>${esc((pools().school[L].subjects && pools().school[L].subjects[k]) ? '' : '')}${esc(CONTENT.school.subjTags[L] && CONTENT.school.subjTags[L][k] || '1:1 맞춤')}</small></a>`; }).join('');
  const subjLines = SITE.subjects.map(k => { const si = subjInfo(k); const line = G.subjectLine(k); return line ? `<li><strong>${si.emoji} ${esc(si.kor)}</strong><span>${esc(line)} <a href="/school/${s.slug}/${k}/">자세히 →</a></span></li>` : ''; }).join('');
  const near = schoolsNear(s, 6);
  const nearHtml = near.length ? `<h2 class="h2">${esc(s.dong)} 인근 학교</h2><p class="p">${esc(s.dong)}과 그 주변에 있는 다른 학교입니다. 같은 동네라도 학교마다 평가 방식이 달라서 학교별로 안내를 따로 두었습니다.</p><div class="pills">${near.map(x => `<a class="pill" href="/school/${x.slug}/">${esc(x.name)}</a>`).join('')}</div>` : '';
...
<h1 class="h1">${esc(s.name)} <em>1:1 맞춤 과외</em><span class="h1-sub">${esc(s.dong)} ${LEVEL_SHORT[L]} 학생 · 학교 평가 방식에 맞춘 수업</span></h1>${updHtml()}
${bodyImg('school-' + s.slug, s.name + ' 1:1 과외', '5/2')}
<div class="lead-p">${G.open}</div>
${join(G.top)}
<h2 class="h2">📚 ${esc(s.name)} 과목별 1:1 과외</h2><p class="p">같은 학교라도 과목마다 평가 형식이 다릅니다. 과목을 고르면 ${esc(s.name)} ${LEVEL_SHORT[L]} 학생 기준 안내로 이어집니다.</p><div class="sch-grid">${subjGrid}</div>${subjLines ? `<ol class="ol">${subjLines}</ol>` : ''}
${join(G.mid)}${regionLink}${nearHtml}${join(G.bottom)}
${faqHtml(G.faqs, `${s.name} 1:1 과외 자주 묻는 질문`)}
${ctaBlock(G.cta)}</article>`;
  return { title: T(`${s.name} 1:1 과외 - ${s.dong} 학교 맞춤 수업`), desc: `${s.sggName} ${s.dong} ${s.name} 학생을 위한 1:1 방문·화상 과외. 학교 평가 방식에 맞춰 4주 전부터 준비. ${SITE.subjects.map(k => subjInfo(k).kor).join('·')}. 첫 상담 무료 ${SITE.tel}`, keywords: `${s.name} 과외,${s.name} 1:1 과외,${s.dong} 학교별 과외,${s.sggName} ${LEVEL_SHORT[L]} 과외`, canonical, breadcrumbs: toAbs(bc, canonical), faqs: G.faqs, ld: serviceLD(`${s.name} 1:1 과외`, `${s.name} 학생 맞춤 1:1 과외`, canonical, `${s.sidoFull} ${s.sggName} ${s.dong}`), body, form: { school: s.name, addr: `${s.sidoFull} ${s.sggName} ${s.dong}`, detail: `${s.name} 학생 과외 상담 신청합니다.` } };
}
...
export function schoolSubjectPage(slug, key) {
  ...
  const L = s.level; const gradeKey = LEVEL_GRADE[L]; const gi = GRADE_INFO[gradeKey]; if (!si.grade || !si.grade[gradeKey]) return null;
  const canonical = ABS(`/school/${s.slug}/${key}/`);
  const seed = `school-${slug}-${key}`;
  const ctx = { sidoFull: s.sidoFull, sgg: s.sggName, dong: s.dong, subj: si.kor, grade: gi.label, school: s.name, level: LEVEL_KOR[L] };
  const G = schoolSections(L, seed, ctx, key);
  if (!G) return null;
...
${bodyImg(`school-${s.slug}-${key}`, `${s.name} ${si.kor} 과외`, '5/2')}
<div class="lead-p">${G.open}</div>
...
${join(G.bottom)}
${faqHtml(G.faqs, `${s.name} ${si.kor} 과외 자주 묻는 질문`)}
${ctaBlock(G.cta)}</article>`;
  return { title: T(`${s.name} ${si.kor} 과외 - ${LEVEL_SHORT[L]} 1:1 맞춤 수업`), desc: `${s.sggName} ${s.dong} ${s.name} ${LEVEL_SHORT[L]} 학생 1:1 ${si.kor} 과외. ${si.short} 학교 평가 방식 맞춤, 첫 상담 무료 ${SITE.tel}`, keywords: `${s.name} ${si.kor} 과외,${s.name} ${si.kor},${s.dong} ${LEVEL_SHORT[L]} ${si.kor} 과외`, canonical, breadcrumbs: toAbs(bc, canonical), faqs: G.faqs, ld: serviceLD(`${s.name} ${si.kor} 과외`, `${s.name} 학생 1:1 ${si.kor} 과외`, canonical, `${s.sidoFull} ${s.sggName} ${s.dong}`), body, form: { school: s.name, subject: si.kor, addr: `${s.sidoFull} ${s.sggName} ${s.dong}`, detail: `${s.name} ${si.kor} 과외 상담 신청합니다.` } };
}
