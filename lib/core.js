...
export function PR(meta, body, form = {}, opts = {}) {
  ...
  return new Response(html, { status: opts.status || 200, headers: { 'Content-Type': 'text/html;charset=UTF-8', 'Cache-Control': opts.cache || 'public, max-age=3600' } });
}
...
export function subjectBox(cur, prefix) {
  return `<div class="subj-box"><b>📚 과목 선택</b><div class="pills">${ALL_SUBJ_KEYS.map(k => { const s = subjInfo(k); return `<a class="pill${k === cur ? ' on' : ''}" href="${prefix}/${k}/">${s.emoji} ${esc(s.kor)}</a>`; }).join('')}</div></div>`;
}
