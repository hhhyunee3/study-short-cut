...
// 본문 이미지: public/images/body/1~12.jpg 중 페이지 주소 해시로 하나를 고른다. 같은 페이지는 늘 같은 사진.
const BODY_IMG_N = 12;
export function bodyImg(seed, alt, ratio = '16/9', extra = '') {
  let h = 0x811c9dc5;
  const s = String(seed || '');
  for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 0x01000193); }
  h ^= h >>> 16; h = Math.imul(h, 0x85ebca6b); h ^= h >>> 13; h = Math.imul(h, 0xc2b2ae35); h ^= h >>> 16;
  const n = ((h >>> 0) % BODY_IMG_N) + 1;
  return imgPH(`/images/body/${n}.jpg`, alt, ratio, extra);
}
...
.only-mo{display:none}.hd-call{display:none}
...
@media(max-width:900px){.only-mo{display:inline-block}.hd-call{display:inline-block}.hd-form{display:none}.burger{display:inline-block}.nav{display:none;position:absolute;left:0;right:0;top:100%;background:#fff;border-bottom:1px solid var(--line);flex-direction:column;align-items:stretch;padding:10px 16px 16px;gap:4px;max-height:80vh;overflow:auto}.nav.open{display:flex}.nm-btn{display:block;width:100%;text-align:left;padding:12px 8px}.nd,.nd-wide{position:static;display:none;box-shadow:none;border:none;padding:4px 8px 10px}.nd.open,.nd-wide.open{display:flex;flex-wrap:wrap;gap:6px}.hd-cta{margin:8px 0 0;text-align:center}}
/* 히어로·섹션 */
...
.rv{background:#fff;border:1px solid var(--line);border-radius:16px;padding:20px;text-align:left}.rv strong{display:block;font-size:.92rem;margin-bottom:8px;color:var(--c1)}.rv p{color:var(--mute);font-size:.92rem;line-height:1.75;margin:0}.rv-grid.rv3{grid-template-columns:repeat(3,1fr)}
...
.h2[class*=" i-"]::before{margin-right:8px;font-size:1.05em;line-height:1;vertical-align:-1px}.i-open::before{content:"📍"}.i-intro::before{content:"📘"}.i-why::before{content:"🔍"}.i-why11::before{content:"🎯"}.i-method::before{content:"🧩"}.i-curri::before{content:"🗂️"}.i-effect::before{content:"🌱"}.i-mistakes::before{content:"⚠️"}.i-exam::before{content:"📝"}.i-level::before{content:"🎓"}.i-routine::before{content:"🔁"}.i-schedule::before{content:"🗓️"}.i-process::before{content:"🚪"}.i-teacher::before{content:"👩‍🏫"}.i-visit::before{content:"🏠"}.i-parent::before{content:"👨‍👩‍👧"}.i-consult::before{content:"☎️"}.i-guide::before{content:"🧭"}.i-habit::before{content:"⏰"}
.sep{height:6px;margin:14px 0 16px;background:radial-gradient(circle,color-mix(in srgb,var(--c3) 45%,#fff) 3px,transparent 3px) repeat-x left center/18px 6px;opacity:.75}
@media(max-width:640px){.sep{margin:12px 0 14px}}
...
.ol li{display:grid;grid-template-columns:44px 1fr;column-gap:12px;row-gap:2px;padding:14px 0;border-bottom:1px dashed var(--line);align-items:start}
...
.ol li::before{grid-column:1;grid-row:1 / span 2}
.ol li>strong,.ol li>span{grid-column:2}
...
// 스타일이 바뀌면 주소(?v=)도 바뀌어 브라우저가 저장해 둔 예전 스타일을 쓰지 않는다
export const STYLES_VER = (() => { let h = 0x811c9dc5; for (let i = 0; i < STYLES_CSS.length; i++) { h ^= STYLES_CSS.charCodeAt(i); h = Math.imul(h, 0x01000193); } return (h >>> 0).toString(36); })();
...
<link rel="icon" type="image/svg+xml" href="/favicon.svg"><link rel="icon" type="image/png" sizes="192x192" href="/favicon.png"><link rel="apple-touch-icon" href="/apple-touch-icon.png">
...
<link rel="stylesheet" href="/styles.css?v=${STYLES_VER}">${m.head || ''}</head><body>`;
}
...
<div class="nm"><a class="nm-btn" href="/guides/">교육정보</a></div>
<a class="hd-cta hd-form" href="#consult-form">무료 상담 신청</a><a class="hd-cta hd-call" href="tel:${SITE.telRaw}">📞 무료 상담</a></nav></div></header>`;
}
export function FTR() {
  const first = SITE.subjects[0];
  return `<footer class="ft"><div class="wrap"><a class="ft-brand" href="/">${logoSvg(26)}${esc(SITE.name)}</a><span style="font-size:.86rem">${esc(SITE.tagline)}</span><a class="ft-tel only-mo" href="tel:${SITE.telRaw}">${SITE.tel}</a>
<nav class="ft-nav" aria-label="푸터 메뉴"><a href="/regions/">전국 지역</a><a href="/seoul/${first}/">지역별 수업</a><a href="/schools/">학교별 수업</a><a href="/${first}/">과목별 수업</a><a href="/tools/grade-calculator/">내신 계산기</a><a href="/guides/">교육정보</a></nav>
...
export const ctaHtml = (h3, p) => `<div class="cta"><h3>${esc(h3)}</h3><p>${esc(p)}</p><div class="btn-row"><a href="#consult-form" class="btn btn-p">상담 신청하기</a><a href="tel:${SITE.telRaw}" class="btn btn-o only-mo">📞 ${SITE.tel}</a></div></div>`;
