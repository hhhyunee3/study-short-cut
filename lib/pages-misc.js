...
import { esc, escAttr, imgPH, subjInfo, ALL_SUBJ_KEYS, ABS, faqHtml, ctaHtml, updHtml, bcHtml, websiteLD, articleLD, formatKR } from './core.js';
...
export function homePage() {
  ...
  const body = `<section class="hero"><div class="wrap"><div><span class="hero-badge">${esc(H.badge)}</span><h1>${H.h1}</h1><p>${esc(H.sub)}</p><div style="display:flex;gap:10px;flex-wrap:wrap"><a class="btn btn-p" href="#consult-form">무료 상담 신청</a><a class="btn btn-o" href="tel:${SITE.telRaw}">📞 ${SITE.tel}</a></div>
...
${imgPH('/images/home/hero.jpg', SITE.name + ' 1:1 과외 수업', '4/3', 'background:rgba(255,255,255,.14)')}</div></section>
...
<section class="sec"><div class="wrap"><span class="sec-tag">REVIEWS</span><h2 class="anim" data-anim="up">수업 후기</h2><p class="lead">${esc(H.reviewsLead)}</p><div class="rv-grid">${[1, 2, 3, 4].map(i => `<div class="rv anim" data-anim="zoom"><img src="/images/reviews/${i}.jpg" alt="수업 후기 ${i}" loading="lazy" onerror="this.remove()" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;border-radius:16px"><span>${esc(H.reviewsNote)}</span></div>`).join('')}</div></div></section>
...
export function guidesHubPage() {
  ...
  const body = `<article class="art">${bcHtml([{ name: '홈', url: '/' }, { name: '정보글' }])}
<h1 class="h1">정보글<span class="h1-sub">시험 준비 · 과목별 공부법 · 과외 고르는 기준</span></h1>${updHtml()}
...
${ctaHtml('읽고 궁금한 점이 남았다면', '상담에서 현재 상황을 듣고 필요한 것만 제안해 드립니다.')}</article>`;
  return { title: T('정보글 - 시험 준비·공부법·과외 고르는 법'), desc: `${SITE.name} 정보글 모음. 시험 준비 순서, 과목별 공부 방법, 과외와 학원 비교처럼 학생과 학부모가 상담에서 자주 묻는 내용을 정리합니다.`, canonical, breadcrumbs: [{ name: '홈', url: ABS('/') }, { name: '정보글', url: canonical }], robots: posts.length ? '' : 'noindex,follow', body, form: { detail: '정보글을 보고 상담 신청합니다.' } };
}
export function guidePostPage(slug) {
  ...
  const body = `<article class="art">${bcHtml([{ name: '홈', url: '/' }, { name: '정보글', url: '/guides/' }, { name: g.h1 }])}
...
${others.length ? `<h2 class="h2">다른 정보글</h2><div class="grid g3">${others.map(gcard).join('')}</div>` : ''}
${ctaHtml(`${g.cta || '공부'} 고민, 상담에서 바로 확인해 보세요`, '현재 상황을 듣고 필요한 것만 제안해 드립니다. 첫 상담은 무료입니다.')}</article>`;
  return { title: `${g.title} | ${SITE.name}`, desc: g.desc, keywords: (g.tags || []).join(','), canonical, ogType: 'article', breadcrumbs: [{ name: '홈', url: ABS('/') }, { name: '정보글', url: ABS('/guides/') }, { name: g.h1, url: canonical }], faqs: g.faq || [], ld: articleLD(g, canonical), body, form: { detail: `정보글 "${g.h1}"을 보고 상담 신청합니다.` } };
}
