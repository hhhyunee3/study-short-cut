// 상담 접수 /api/contact — 이메일(Email Routing) + 구글 시트 이중 기록. 한 곳이라도 성공해야 접수 완료.
...
export async function handleContact(request, env) {
  ...
  let mailOk = false, sheetOk = false, err = '';
  ...
  console.log('상담 신청', SITE.name, '| 메일:', mailOk, '| 시트:', sheetOk, '| 페이지:', r.page);
  if (!mailOk && !sheetOk) return json({ ok: false, error: `접수 중 문제가 생겼습니다. 전화 ${SITE.tel} 로 연락 주시면 바로 도와드리겠습니다.` }, 500);
  return json({ ok: true });
}
