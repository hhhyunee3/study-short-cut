...
export async function handleContact(request, env) {
  ...
  let mailOk = false, sheetOk = false, tgOk = false, err = '';
  ...
  console.log('상담 신청', SITE.name, '| 메일:', mailOk, '| 시트:', sheetOk, '| 텔레그램:', tgOk, '| 페이지:', r.page);
  if (!mailOk && !sheetOk && !tgOk) return json({ ok: false, error: `접수 중 문제가 생겼습니다. 전화 ${SITE.tel} 로 연락 주시면 바로 도와드리겠습니다.` }, 500);
  return json({ ok: true });
}
